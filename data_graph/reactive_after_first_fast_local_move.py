...
@njit(cache=False, nogil=True)
def _optimize_empty_aware_newman_reactive_after_first_core(
    indptr,
    indices,
    data,
    initial_labels,
    ...
    # Hybrid scheduling:
    #   pass 0: canonical-order full sweep;
    #   later passes: only nodes activated by actual moves.
    #
    # Full canonical convergence is intentionally performed by the
    # untouched canonical mover after this bounded reactive phase.
    active = np.zeros(n, dtype=np.uint8)
    frontier_pass_limit = 8
    hybrid_pass_limit = 1 + frontier_pass_limit

    if max_passes < hybrid_pass_limit:
        hybrid_pass_limit = max_passes
    ...
    for pass_index in range(
        ...
        for u in range(n):
            ...
            # Actual movement seeds the next reactive pass.
            active[u] = 1
            for frontier_p in range(
                indptr[u],
                indptr[u + 1],
            ):
                active[indices[frontier_p]] = 1
...
def optimize_empty_aware_newman_reactive_after_first(
    indptr,
    indices,
    data,
    initial_labels,
    ...
    (
        ...
        moves,
        empty_moves,
    ) = _optimize_empty_aware_newman_reactive_after_first_core(
        indptr,
        indices,
        data,
        initial_labels,
