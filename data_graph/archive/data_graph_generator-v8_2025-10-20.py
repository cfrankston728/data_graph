...
from .knn_dim_reduction import reduce_for_knn, KNNReductionConfig
...
@njit(parallel=True)
def _fill_csr_with_rowwise_euclid(indptr, indices, E, out_data):
    """
    Fill CSR weights with Euclidean distances row-by-row from embedding matrix E (float32/float64).
    """
    n, d = E.shape
    for i in prange(n):
        start = indptr[i]
...
class DataGraphGenerator:
    ...
    def create_knn_graph_with_mst(self, 
        ...
        cfg = self.knn_reduction
        if cfg is None:
            try:
                # Default: PCA targeting ~98% explained variance (lets reduce_for_knn pick n_components)
                cfg = KNNReductionConfig(method="ipca", var=0.98, n_components=None,
                                        random_state=0, cast_float32=True)
            except Exception:
                cfg = None  # reduce_for_knn will treat None as "none"

        X_knn, dr_info = reduce_for_knn(embeddings, cfg, verbose=self.verbose)
        ...
        weights_from = self.knn_weights_from  # "original" | "reduced"
